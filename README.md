...
```
llm_summarization_br_ca/
├── notebooks/              7 Jupyter notebooks (01–07) with MSK | Goel Lab headers
├── data/
│   ├── raw/                Validation dataset (deidentified)
│   ├── processed/          Enhanced metrics CSVs, prompt libraries
│   ├── deidentified/       Redacted PDFs (generated by NB01)
│   ├── extracted_text/     OCR text per case (generated by NB04)
│   ├── features/           BERT embeddings, OCR quality, text features (NB05)
...
├── .env.example            API key template (OpenAI, Anthropic, Phoenix)
...
1. Open any notebook in VS Code or JupyterLab
2. Select the `.venv` kernel
3. Run cells — all paths are configured via `PROJECT_ROOT`
